...
$apiInstance = new ACBrAPI\Api\NfceApi(
...
```php
cancelarNfce($id, $body): \ACBrAPI\Model\DfeCancelamento
```
...
```php
consultarCancelamentoNfce($id): \ACBrAPI\Model\DfeCancelamento
```
...
```php
consultarEventoNfce($id): \ACBrAPI\Model\DfeEvento
```
...
```php
consultarInutilizacaoNfce($id): \ACBrAPI\Model\DfeInutilizacao
```
...
```php
consultarLoteNfce($id): \ACBrAPI\Model\DfeLote
```
...
```php
consultarNfce($id): \ACBrAPI\Model\Dfe
```
...
```php
consultarStatusSefazNfce($cpf_cnpj, $autorizador): \ACBrAPI\Model\DfeSefazStatus
```
...
```php
emitirLoteNfce($body): \ACBrAPI\Model\DfeLote
```
...
```php
emitirNfce($body): \ACBrAPI\Model\Dfe
```
...
```php
inutilizarNumeracaoNfce($body): \ACBrAPI\Model\DfeInutilizacao
```
...
```php
listarEventosNfce($dfe_id, $top, $skip, $inlinecount): \ACBrAPI\Model\DfeEventoListagem
```
...
```php
listarLotesNfce($cpf_cnpj, $ambiente, $top, $skip, $inlinecount, $referencia): \ACBrAPI\Model\DfeLoteListagem
```
...
```php
listarNfce($cpf_cnpj, $ambiente, $top, $skip, $inlinecount, $referencia, $chave, $serie): \ACBrAPI\Model\DfeListagem
```
...
```php
sincronizarNfce($id): \ACBrAPI\Model\DfeSincronizacao
```
