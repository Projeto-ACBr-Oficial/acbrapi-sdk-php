...
$apiInstance = new ACBrAPI\Api\DceApi(
...
```php
cancelarDce($id, $body): \ACBrAPI\Model\DfeCancelamento
```
...
```php
consultarCancelamentoDce($id): \ACBrAPI\Model\DfeCancelamento
```
...
```php
consultarDce($id): \ACBrAPI\Model\Dfe
```
...
```php
consultarStatusSefazDce($cpf_cnpj, $autorizador): \ACBrAPI\Model\DfeSefazStatus
```
...
```php
emitirDce($body): \ACBrAPI\Model\Dfe
```
...
$body = new \ACBrAPI\Model\DcePedidoEmissao(); // \ACBrAPI\Model\DcePedidoEmissao
...
| Nome | Tipo | Descrição  | Notas |
| ------------- | ------------- | ------------- | ------------- |
| **body** | [**\ACBrAPI\Model\DcePedidoEmissao**](../Model/DcePedidoEmissao.md)|  | |
...
```php
listarDce($cpf_cnpj, $ambiente, $top, $skip, $inlinecount, $referencia, $chave, $serie): \ACBrAPI\Model\DfeListagem
```
