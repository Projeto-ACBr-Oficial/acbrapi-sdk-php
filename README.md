...
```json
{
  "repositories": [
    {
      "type": "vcs",
      "url": "https://github.com/projeto-acbr-oficial/acbrapi-sdk-php.git"
    }
  ],
  "require": {
    "projeto-acbr-oficial/acbrapi-sdk-php": "*@dev"
  }
}
```
...
```php
<?php
require_once('/path/to/ACBrAPI/vendor/autoload.php');
```
...
$config = ACBrAPI\Configuration::getDefaultConfiguration()->setAccessToken($token->access_token);
...
A função `get_oauth2_token` é uma função genérica para buscar o token OAuth2 a partir das credenciais. Isso não é definido pela ACBr API,
...
- Versão da API: `2.57.5`
    - Versão do package: `2.57.5`
